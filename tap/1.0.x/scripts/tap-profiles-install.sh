...
if [[ -z $(tanzu package installed list -n tap-install -o yaml | yq '.[] | select(.name == "tap")') ]]; then
  ...
  tanzu package install tap -p tap.tanzu.vmware.com -v 1.0.0 --values-file $TAP_VALUES_FILE -n tap-install
else
  echo "## Updating a Tanzu Application Platform profile"
  tanzu package installed update tap -p tap.tanzu.vmware.com -v 1.0.0 --values-file $TAP_VALUES_FILE -n tap-install
fi
